...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
    ...
    UNUserNotificationCenter.current().requestAuthorization(options: [.alert, .badge, .sound]) { (success, error) in
        ...
        success ? print("Successfully authorized to send push notification") : print("Denied, Can't send this person notifications")
    ...
    func checkAccountStatus(completion: @escaping (Bool) -> Void) {
        CKContainer.default().accountStatus { (staus, error) in
            if let error = error {
                print(error.localizedDescription)
                ...
                DispatchQueue.main.async {
                    ...
                    case .couldNotDetermine:
                        tabBarController?.presentSimpleAlertWith(title: errorText, message: "There was an unkown erro fetching your iCloud Account")
                    case .available:
                        completion(true)
                    case .restricted:
